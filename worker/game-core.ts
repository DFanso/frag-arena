...
export class GameRoomCore {
  ...
  private addPlayer(ws: Conn, name: string, rawToken?: string): void {
    ...
    const rec: PlayerRec = {
      ...
      // Restore the full match economy on reconnect, not just frags/deaths — otherwise a drop+rejoin
      // within the grace window refreshes to STARTING_CREDITS (free-refill exploit) and silently loses
      // bought weapons (review fix #12). Fresh joiners get the defaults.
  ...
  private addBot(name: string): void {
    ...
    const rec: PlayerRec = {
      ...
      hp: MAX_HP, st: ST_ALIVE, frags: 0, deaths: 0, credits: STARTING_CREDITS, ownedWeapons: defaultOwnedWeapons(),
